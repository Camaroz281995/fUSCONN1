...
export async function POST(request: NextRequest) {
  try {
    ...
    const existing = await sql`SELECT 1 FROM profile_community_members WHERE community_id = ${communityId} AND username = ${username}`
    if (existing.length > 0) {
      return NextResponse.json({ error: 'Already a member' }, { status: 400 })
    }
    await sql`INSERT INTO profile_community_members (community_id, username) VALUES (${communityId}, ${username})`
