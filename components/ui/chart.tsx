...
// NOTE: recharts is loaded dynamically to prevent its internal <style> tag
// injection from triggering React's script-tag warning during SSR/hydration.
...
function buildCssVars(config: ChartConfig): React.CSSProperties {
  ...
  for (const [key, val] of Object.entries(config)) {
    const color = val.theme?.light ?? val.theme?.dark ?? val.color
...
export const ChartContainer = React.forwardRef<HTMLDivElement, ChartContainerProps>(
  ({ id, className, children, config, ...props }, ref) => {
    ...
    const cssVars = React.useMemo(() => buildCssVars(config), [config])
...
// ─── ChartStyle (no-op) ───────────────────────────────────────────────────────
export const ChartStyle = (_props: { id: string; config: ChartConfig }) => null

// ─── ChartTooltip ─────────────────────────────────────────────────────────────
// Re-exported lazily — consumers that use recharts directly will still work
// because they import recharts themselves.
// ChartTooltip — safe pass-through; recharts Tooltip used directly by consumers
export const ChartTooltip = (_props: Record<string, unknown>) => null
...
// ─── ChartLegend ──────────────────────────────────────────────────────────────
export const ChartLegend = (_props: Record<string, unknown>) => null
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
  ...
  let configKey = key
  if (!(key in config) && inner && key in (inner as object)) {
